...
def make_players_table(data, cur, conn):
    # Create the Players table with the specified columns
    cur.execute("""
    ...
    # Iterate through the JSON data to get the list of players in the squad
    players = data['squad']
    for player in players:
        # Extract the player information from the JSON object
        player_id = player['id']

        # Check if the player ID already exists in the Players table
        cur.execute("SELECT COUNT(*) FROM Players WHERE id=?", (player_id,))
        if cur.fetchone()[0] > 0:
            continue  # skip this player if the ID already exists

        player_name = player['name']
        player_position = player['position']
        player_birthyear = player['dateOfBirth'][:4]
        player_nationality = player['nationality']

        # Look up the position_id for the player in the Positions table
        ...
        cur.execute("""
            INSERT INTO Players (id, name, position_id, birthyear, nationality)
            VALUES (?, ?, ?, ?, ?)
        """, (player_id, player_name, position_id, player_birthyear, player_nationality))
...
def birthyear_nationality_search(age, country, cur, conn):
    ...
    earliest_birth_year = 2023 - age

    # Query the database for all players from the country with birth years before the earliest birth year
    query = """
        SELECT Players.name, Players.nationality, Players.birthyear
        FROM Players
        WHERE Players.nationality = ? AND Players.birthyear < ?
    """
    cur.execute(query, (country, earliest_birth_year))
    rows = cur.fetchall()
...
def position_birth_search(position, age, cur, conn):
    ...
    cur.execute(query, (position, earliest_birth_year))
